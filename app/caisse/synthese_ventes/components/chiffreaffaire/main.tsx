...
export default function ChiffreAffaire({
  ...
  useEffect(() => {
    ...
    if (viewType === "yearly") {
      filteredData = salesData.filter((item: any) => item.total_ttc !== 0);
    }
    ...
    <div className="space-y-4 overflow-x-auto">
      ...
      <table className="w-full ">
        <thead className="bg-slate-800 text-white">
          <tr>
            <th className="p-4 border rounded-tl-lg">Date Creation</th>
        ...
        <tbody className="text-center">
          {data.map((data: any, index: number) => (
            ...
            >
              ...
              <td className="p-4 text-slate-800">
                {Number(data?.total_tva_v0)}
              </td>
              <td className="p-4 text-slate-800">
                {Number(data?.total_tva_v20)}
              </td>
              <td className="p-4 text-slate-800">
                {Number(data?.total_tva_v21)}
              </td>
